...
function WT.Gadget.ShowCreationUI()
	if not WT.Gadget.CreateGadgetWindow then
		...
		if wtxOptions.BackgroundTex == "BackgroundTexTrans" then
			window:SetTexture(AddonId, "img/menu2.png")
		else
			window:SetTexture(AddonId, "img/menu.png")
		end
		...
		window.closeButton:SetTexture("Rift", "btn_gold_close_(over).png.dds" )
		...
		coroutine.resume(co)
		Command.Event.Attach(WT.Event.Tick,
			function()
