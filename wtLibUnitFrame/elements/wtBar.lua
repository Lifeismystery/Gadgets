...
function wtBar:Construct()
	...
	if config.BarAlphaBinding then
		self.UnitFrame:CreateBinding(config.BarAlphaBinding, self, self.BindBarAlpha, nil)	
	end
	if config.BarTextureBinding then
		self.UnitFrame:CreateBinding(config.BarTextureBinding, self, self.BindBarTexture, nil)
	end
...
function wtBar:BindBarAlpha(BarAlpha)
    if BarAlpha then
		self:SetAlpha(BarAlpha.alpha or 1)
	end	
end
function wtBar:BindBarTexture(BarTexture)
    if BarTexture then
		if BarTexture.media then
			Library.Media.SetTexture(self.Image, BarTexture.media)
		elseif BarTexture.texAddon and BarTexture.texFile then
			self.Image:SetTexture(BarTexture.texAddon, BarTexture.texFile)
		end
	end	
end